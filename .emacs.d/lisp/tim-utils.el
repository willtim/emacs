;;; tim-utils.el --- Define some custom functions
;;
;; many taken from the Emacs Starter Kit
 
(require 'thingatpt)
(require 'imenu)
 
...
 
(defun dos-shell ()
  "Start a DOS (cmd.exe) shell"
  (interactive)
  (let ((explicit-shell-file-name "c:\\winnt\\system32\\cmd.exe")
        (buf-name "*dos-shell*"))
    (shell buf-name)))
 
;; Excel helpers
;;;;;;;;;;;;;;;;;;;;
 
(defun time-from-excel-date (x)
"Excel date to time (non-interactive)"
(encode-time 0 0 0 (+ 1 (- x 25934) ) 1 1971))
 
(defun ed (x)
  "Excel date to time"
  (interactive "NEnter an Excel date as an integer: ")
  (print (format-time-string "%d %b %Y" (time-from-excel-date x))))
 
;; Network
 
(defun view-url ()
